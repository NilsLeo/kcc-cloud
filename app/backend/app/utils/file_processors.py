...
def process_7z(file_path: str, temp_dir: str, job_id: str = None, user_id: str = None) -> str:
    ...
    try:
        ...
        log_with_context(
            logger,
            "info",
            f"Successfully extracted 7Z/CB7 with {file_count} files to {nested_dir}",
            job_id=job_id,
            user_id=user_id,
            file_path=file_path,
...
def process_zip(file_path: str, temp_dir: str, job_id: str = None, user_id: str = None) -> str:
    ...
    try:
        ...
        log_with_context(
            logger,
            "info",
            f"Successfully extracted ZIP/CBZ with {file_count} files to {manga_dir}",
            job_id=job_id,
            user_id=user_id,
            file_path=file_path,
            file_count=file_count,
...
def process_rar(file_path: str, temp_dir: str, job_id: str = None, user_id: str = None) -> str:
    ...
    try:
        ...
        log_with_context(
            logger,
            "info",
            f"Successfully extracted RAR/CBR with {file_count} files to {nested_dir}",
            job_id=job_id,
            user_id=user_id,
            file_path=file_path,
            file_count=file_count,
...
def unwrap_nested_archives(
    extracted_dir: str, job_id: str = None, user_id: str = None, max_depth: int = 10
) -> str:
    """
    ...
    while depth < max_depth:
        ...
        all_files = [
            f
            for f in glob.glob(os.path.join(current_dir, "**/*"), recursive=True)
            if os.path.isfile(f)
        ]
        ...
        if len(all_files) > 1:
            log_with_context(
                logger,
                "info",
                f"Nested archive unwrapping complete: found {len(all_files)} files",
                job_id=job_id,
                user_id=user_id,
        ...
        log_with_context(
            logger,
            "info",
            f"Found nested archive: {os.path.basename(single_file)} - extracting...",
            job_id=job_id,
            user_id=user_id,
        ...
        try:
            if file_ext in {".zip", ".cbz"}:
                nested_dir = process_zip(
                    single_file, new_extract_dir, job_id=job_id, user_id=user_id
                )
            elif file_ext in {".rar", ".cbr"}:
                nested_dir = process_rar(
                    single_file, new_extract_dir, job_id=job_id, user_id=user_id
                )
            elif file_ext in {".7z", ".cb7"}:
                nested_dir = process_7z(
                    single_file, new_extract_dir, job_id=job_id, user_id=user_id
                )
            else:
            ...
            log_with_context(
                logger,
                "error",
                f"Failed to extract nested archive: {str(e)}",
                job_id=job_id,
                user_id=user_id,
                nested_archive=os.path.basename(single_file),
    ...
    if depth >= max_depth:
        log_with_context(
            logger,
            "warning",
            f"Nested archive unwrapping stopped: max depth ({max_depth}) reached",
            job_id=job_id,
            user_id=user_id,
            current_dir=current_dir,
        )
